...
Enable =mixed-pitch-mode= for text modes. Also ensure it preserves the variable pitch height rather than inheriting from
the default face.
...
project roots over =.git=. Also, prefer =.project= over anything else by placing it first in the root files.. _I'm
finding this too weird - I like having .git be the root of my project. The main issue I was trying to solve was with LSP
misidentifying roots, but I'll just leave the initial guess root to nil for now._
...
entire ~lsp-command-map~ provided by LSP to =SPC c l=. _No longer needed - this is now in doom!_
