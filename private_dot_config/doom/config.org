...
This file is a literate org-mode file which generates my =config.el= and =packages.el= files. For doom reasons, =init.el= cannot be generated from a literate file, so I keep that separate.

The source for =config.org= (this file) and =init.el= is located in my [[https://github.com/jsravn/dotfiles/tree/master/private_dot_config/doom][dotfiles]].
...
* Modules
This section configures the modules enabled in my [[https://github.com/jsravn/dotfiles/blob/master/private_dot_config/doom/init.el][init.el]]. Languages have their own [[*Languages][dedicated section]].
...
**** Visuals
Use mixed pitch so headings appear larger. I've disabled this until I can shrink the variable pitch font.
